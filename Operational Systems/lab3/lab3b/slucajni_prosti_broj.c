...
void stavi_u_MS(uint64_t broj)
{
  MS[ULAZ] = broj;
	ULAZ = (ULAZ + 1)%6;
	BROJAC++;
	if(BROJAC > 6){ //ako prepisuje nepročitanu poruku
		BROJAC--;
		IZLAZ = (IZLAZ + 1)%6;
	}
  return;
...
void Udji_u_KO(void *id){
/*  do{}while( ZASTAVICA == 1 );
  ZASTAVICA = 1; */
  return;
};


void Izadji_iz_KO(void *id){
/*  ZASTAVICA = 0; */
  return;
};

void ispisi_stanje_u_MS(){
  printf(" Ulaz=%d Izlaz=%d", ULAZ, IZLAZ );
  return;
}
